...
ats-hbase, ambari metrics collector 集群清理数据
merge 小region时经常需要频繁查看region大小，相邻位置来执行merge命令，效率比较低，做此工具提升效率
## 2. 用法
```
支持的操作有get, scan， merge
show_usage="args:
-a --action=[list/put/get/scan/Tmerge/merge/alter/major_compact/truncate/truncate_preserve],
-n --namespace=]
-t --tablename=]
...
-l --lowsizeperregion, the max hfile size of per region joined in merge group, unit MB
-u --upperregionsize, the max hfile size of new region after merge, unit MB
...
### 2.3 merge
```
   Tmerge 合并测试，常用于merge 操作前测试验证
   将hfile 小于500MB, 单次合并region个数不超过2个，合并region后的hfile大小不会超过2048MB，常用于判断哪些region将会被合并
...
```
    ...
    或者从./log/data 中批量读取put 数据，每10个put为一组，文件内容格式为
    row1,cf:f1,v1
    row2,cf:f2,v2
